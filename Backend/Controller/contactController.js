...
export const handleAddContact = async (request, response) => {
  try {
    ...
    // Emit real-time notification to admins about new contact message
    const io = getIO();
...
export const handleGetAllContacts = async (request, response) => {
  try {
    ...
    return response
      .status(500)
      .json({ message: "Could not retrieve messages" });
  }
};
...
export const handleUpdateContactStatus = async (request, response) => {
  try {
    ...
    // 🔥 EMIT UPDATE
    const io = getIO();
    io.emit("contactUpdated", updated);
...
export const handleDeleteContact = async (request, response) => {
  ...
  // 🔥 EMIT DELETE EVENT
    ...
    return response
      .status(200)
      .json({ message: "Message deleted successfully", id: deleted.id });
  } catch (error) {
    return response
      .status(500)
