  ...
  const AdminContactManager = () => {
    ...
    useEffect(() => {
      ...
      socket.on("newContact", (data) => {
        ...
        triggerToast("New message received!", "success");
      });

      socket.on("contactUpdated", (updated) => {
        setContacts((prev) =>
  ...
  if (wrapper) {
      ...
      setAlertConfig({
        ...
        onConfirm: async () => {
          ...
          try {
            ...
            await Promise.all(
              selectedIds.map((id) =>
                axiosInstance.delete(API_PATHS.CONTACTS.DELETE(id)),
              ),
            );
            setContacts((prev) =>
            ...
            triggerToast("Bulk delete failed", "error");
          }
        },
      });
    };
    ...
    const handleUpdateStatus = async (id, newStatus) => {
      try {
        ...
        await axiosInstance.patch(API_PATHS.CONTACTS.UPDATE_STATUS(id), {
          status: newStatus,
        });
        setContacts((prev) =>
      ...
      <>
        <div className="admin-container">
          <div className="stats-grid">
            <div
              className={`stat-card ${filterMode === "total" ? "active-filter" : ""}`}
              onClick={() => {
                setFilterMode("total");
                setCurrentPage(1);
              }}
            ...
            <div
              className={`stat-card ${filterMode === "unread" ? "active-filter" : ""}`}
              onClick={() => {
          ...
          <div className="admin-header">
            <div>
              ...
              <p className="subtitle">
                Manage messages from potential students and partners
              </p>
              {selectedIds.length > 0 && (
                <button
                  className="delete-btn bulk-btn"
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {paginatedData.map((c) => (
                    <tr
                      key={c.id}
                      className={`${selectedIds.includes(c.id) ? "selected-row" : ""} ${c.status === "unread" ? "unread-row" : ""}`}
                    >
                      <td>
                        <input
                          type="checkbox"
        ...
        <CustomAlerts
          ...
          onCancel={() => setAlertConfig((p) => ({ ...p, isOpen: false }))}
        />
        {toastConfig.show && (
          <Toast
            message={toastConfig.message}
            type={toastConfig.type}
            onClose={() => setToastConfig((p) => ({ ...p, show: false }))}
          />
        )}
      </>
    );
